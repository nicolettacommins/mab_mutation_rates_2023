...
def write_fasta(chromosome, outFile, refID, sample):
    record = SeqRecord(Seq("".join(chromosome), Gapped(IUPAC.ambiguous_dna, '-')), id=refID, description=sample) #changed ID to refID so will be compatible with bedtools maskfasta
